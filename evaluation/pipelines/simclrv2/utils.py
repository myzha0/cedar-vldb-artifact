...
from ember_benchmarks.utils import Downloader
...
def download_dataset():
    ...
    data_dir = (
        pathlib.Path(__file__).resolve().parents[1].joinpath(DATASET_LOC)
    )
    ...
    logging.info("Downloading Imagenette2 Dataset")
    downloader = Downloader(DATASET_NAME)
    downloader.download_dataset(DATASET_SOURCE, DATASET_FILE)

    tar_path = dataset_file.parent

    logging.info("Extract Imagenette2 data from tar file.")
    with tarfile.open(dataset_file, "r:gz") as tar:
        tar.extractall(path=tar_path)
    logging.info("Done extracting Imagenette2 data from tar file.")

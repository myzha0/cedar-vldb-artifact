...
from ember.config import EmberContext, RayConfig
...
from ember.sources import IterSource, LocalFSSource, LocalLineSource
from ember.sources.iterable import InProcessIterSourcePipeVariant
...
def test_profiler(basic_noop_feature):
    basic_noop_feature.load(EmberContext())
...
def test_dynamic_mutate_basic(v_type):
    data = [1, 2, 3, 4, 5]
    source = IterSource(data)
    ctx = EmberContext(RayConfig(n_cpus=16))

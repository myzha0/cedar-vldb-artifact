...
from ember.client.controller import ControllerThread
from ember.client.profiler import FeatureProfiler
...
from ember.sources import IterSource, LocalLineSource, LocalFSSource, source
...
def noop_profiled_feature():
    ...
    test_feature = TestFeature()
    test_feature.apply(source)
    test_feature.load(EmberContext())
...
def override_control_loop_period(monkeypatch):
    monkeypatch.setattr("ember.client.controller.CONTROLLER_PERIOD_SEC", 0.01)
...
def fully_iterated_over_iterable_source_pipe(request):
    ...
    pipe.mutate(EmberContext(), PipeVariantType.INPROCESS)

...
from ember.sources import Source
from ember.pipes.optimize import OptimizerPipeRegistry, FusedOptimizerPipe
...
class Feature(abc.ABC):
    ...
    def load(
        ...
        Args:
            ctx: EmberContext for this session
    ...
    def load_from_plan(
        self, ctx: EmberContext, plan: PhysicalPlan
    ) -> Iterable[Any]:
        """
    ...
    def load_from_yaml(self, ctx: EmberContext, path: str) -> Iterable[Any]:
        """
    ...
    def load_from_dict(self, ctx: EmberContext, d: Dict):
        ...
        if len(source_pipes) < 1:
            ...
            logging.warning(
                "CAREFUL: Detected more than 1 source pipe.\
                            Ember currently does not offer support for more\
                            than 1 source pipe."
            )

        return source_pipes

    def _insert_prefetch(self, ctx: EmberContext) -> None:
    ...
    def profile_tf(self, ctx: EmberContext) -> bool:
        if self.loaded:
            raise RuntimeError("Cannot profile a loaded feature.")

...
from ember.pipes.optimize import OptimizerPipeRegistry
from .utils import (
...
from ember.compose import constants
...
class Optimizer:
        ...
        for p_id in self.physical_plan.graph:
            ...
            if (
                ...
                submit_batch_size = min(
                    int(RAY_SUBMIT_BATCH_SCALING_FACTOR // total_io_size), 500
                )
                logger.info(
                    f"  Input size: {input_size}, output size: {output_size}"
    ...
    def _validate_stats(self) -> None:
        ...
        if self.options.enable_offload and (
            ...
            raise RuntimeError("Profiled stats do not contain Ray data.")
    ...
    def _calculate_caching_plans(
        ...
        NOTE: non-linear graphs are in general not currently supported
        across ember; big TODO to add this feature eventually...

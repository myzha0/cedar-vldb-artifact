...
from ember.compose import Feature, OptimizerOptions, PhysicalPlan
...
class DataSet:
    """
    A DataSet is the user (i.e., ML job) facing API of ember.
    ...
    Args:
        ctx: EmberContext containing runtime context.
    ...
    Attributes:
        ...
        profiled_data: Dict mapping feature name to path to YAML with
            profiled results.
        profile_mode: If true, only run profiling and exit immediately
    """
    ...
    def __init__(
        ...
        self._log_file = pathlib.Path("/tmp/ember_log.txt")
        # Overwrite the file
        ...
        if run_profiling:
            ...
            for f_name in self.feature_names:
                ...
                self.features[f_name].to_yaml(f"/tmp/ember_{f_name}_plan.yml")
            exit(0)
    ...
    def _run_optimizer(self, profiled_data: Optional[str]):
        ...
        # Save the plan
        save_path = "/tmp/ember_optimized_plan.yml"
